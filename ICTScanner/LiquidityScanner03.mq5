...
void ScanSymbol(string sym)
{
   ...
   // 1. DÉFINIR LE "PLAFOND RÉCENT"
   // Quel est le point le plus haut atteint par le prix ces dernières minutes ?
   // (Sur les 'InpSweepMemory' dernières bougies)
   double recentMaxHigh = 0;
   int recentMaxIndex = 0; // On garde l'index de la bougie qui a fait le sweep
   ...
   // 2. TROUVER LE MEILLEUR SOMMET FRACTAL QUI A ÉTÉ CASSÉ PAR CE PLAFOND
   int bestHighIndex = -1;
   double maxBrokenHighPrice = -1.0;
   
   // On commence la boucle APRÈS la zone de mémoire pour ne pas se détecter soi-même
   for(int i = InpSweepMemory + 1; i < InpLookBack; i++)
   {
      if(IsFractalUp(rates, i))
      {
         ...
         // EST-CE QUE CE FRACTAL A ÉTÉ CASSÉ RÉCEMMENT ?
         // Comparaison avec le plus haut des X dernières bougies
         if(fractalPrice <= recentMaxHigh) // Oui, on est passé au-dessus récemment
         {
             // On garde le Fractal le PLUS HAUT parmi ceux cassés (Le Boss)
   ...
   if(bestHighIndex == -1) return; // Aucun sommet cassé trouvé

   // 3. TROUVER LA CIBLE (LOW)
   // Le plus bas entre ce sommet identifié et MAINTENANT (index 0)
   int bestLowIndex = -1;
   ...
   // Si on a déjà clôturé sous la target, c'est fini (setup invalidé)
   if(rates[0].close < minLowPrice) return;

   // 4. ACTION
   // Note: Pour le dessin, le temps du "Sweep" est celui de la bougie 'recentMaxIndex'
   // (Celle qui a fait la mèche la plus haute récemment)
   if(InpDrawRemote)
   {
       DrawOnAllCharts(sym, 
   ...
   {
      string msg = "SMC SWEEP (Recent): " + sym;
      if(InpUseAlert) Alert(msg);
      // On enregistre l'heure de la bougie qui a sweepé pour ne pas répéter l'alerte
