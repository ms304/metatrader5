...
//| AVEC TRADING AUTOMATIQUE ET DEBUG COMPLET                        |
//+------------------------------------------------------------------+
#property version "4.09"
#property copyright "Copyright 2025, SMC EA"

// --- Paramètres d'entrée ---
input string API_KEY = "Ou vé ganye larzan ou là !";
...
string lastResponse = "";               // Stocke la dernière réponse pour débogage
...
void ManagePositions()
{
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(UseBreakeven && profitPips >= BreakevenPips && sl < openPrice)
      {
         ...
         request.symbol = _Symbol;
         request.sl = openPrice + (int)(5 * point);
...
//+------------------------------------------------------------------+
//| Vérification améliorée du pattern                                |
//+------------------------------------------------------------------+
bool IsPatternValid(string response)
{
   ...
   if(DebugMode) Print("Analyse du pattern...");
   
   // Vérifier les mots-clés de validité
   bool isValid = (StringFind(upperResponse, "VALID") >= 0 ||
                   StringFind(upperResponse, "CONFIRMED") >= 0 ||
                   StringFind(upperResponse, "COMPLETE") >= 0 ||
                   StringFind(upperResponse, "SETUP IS VALID") >= 0 ||
                   StringFind(upperResponse, "PATTERN IS CONFIRMED") >= 0);
   
   // Vérifier les mots-clés d'invalidation
   bool isInvalid = (StringFind(upperResponse, "INVALID") >= 0 ||
                     StringFind(upperResponse, "INCOMPLETE") >= 0 ||
                     StringFind(upperResponse, "NOT MET") >= 0 ||
                     StringFind(upperResponse, "DO NOT ENTER") >= 0 ||
                     StringFind(upperResponse, "NO TRADE") >= 0);
   
   // Mode partiel
   if(AllowPartialPattern && !isInvalid && StringFind(upperResponse, "LONG") >= 0)
   {
      if(DebugMode) Print("Pattern partiel accepté (AllowPartialPattern=true)");
      return true;
   }
   
   if(isValid && !isInvalid)
   {
      if(DebugMode) Print("Pattern VALIDE détecté");
      return true;
   }
   
   if(DebugMode) Print("Pattern INVALIDE ou INCOMPLET détecté");
   return false;
}
...
//+------------------------------------------------------------------+
//| Fonction principale CallGemini (avec trading et debug)           |
//+------------------------------------------------------------------+
void CallGemini()
{
   ...
   "If this exact pattern is confirmed and it is time to enter a LONG trade towards the BSL, respond EXACTLY in this format:\n" +
   "DECISION: LONG\n" +
   "ENTRY: X.XX\n" +
   "SL: X.XX\n" +
   "TP: X.XX\n" +
   "REASON: brief explanation\n\n" +
   "If the pattern is incomplete, invalid, or bearish, respond EXACTLY in this format:\n" +
   "DECISION: WAIT\n" +
   "REASON: brief explanation\n\n" +
   ...
   if(DebugMode)
   {
      ...
      Print("Pattern VALIDE ? ", IsPatternValid(finalResponse));
      
      double entryTest = 0, slTest = 0, tpTest = 0;
      bool extracted = ExtractTradeLevels(finalResponse, entryTest, slTest, tpTest);
      ...
      if(entryTest > 0)
         Print("Distance entry/prix: ", MathAbs(ask - entryTest) / point, " points");
   ...
   if(IsPatternValid(finalResponse))
   {
      ...
      if(ExtractTradeLevels(finalResponse, entry, sl, tp))
      {
         ...
         if(!alreadyExecuted && CanTrade())
         {
            ...
            // Vérifier que le prix actuel est proche de l'entrée suggérée (tolérance 100 points)
            if(MathAbs(ask - entry) <= 100 * point)
            {
            ...
            else
            {
               ...
               Print("Distance: ", MathAbs(ask - entry) / point, " points (max 100 points autorisé)");
            }
         }
      ...
      else
      {
         ...
         Print("Réponse reçue: ", StringSubstr(finalResponse, 0, 500));
      }
   }
   else if(AllowPartialPattern && StringFind(finalResponse, "LONG") >= 0)
   {
      Print(">>> PATTERN PARTIEL DETECTE (mode partiel activé) <<<");
      Print("Aucun trade exécuté car les niveaux Entry/SL/TP sont requis");
   }
   else
   {
      Print(">>> PATTERN INVALIDE OU INCOMPLET DETECTE PAR GEMINI <<<");
...
int OnInit()
{
   ...
   Print("Version 4.09 - Debug Mode: ", DebugMode ? "ON" : "OFF");
   Print("========================================");
   ...
   if(!IsConnected())
      Print("ATTENTION: Pas de connexion Internet detectee!");
...
bool IsConnected()
{
   // Test simple: essayer de récupérer le temps du serveur NTP
