...
//| AVEC TRADING LONG ET SHORT - VERSION 5.00                        |
//+------------------------------------------------------------------+
#property version "5.00"
#property copyright "Copyright 2025, SMC EA"

// --- Paramètres d'entrée ---
input string API_KEY = "Té mounwar, kwa fé don ? Ou rod fé larzan kwé ?";
...
bool ExtractTradeLevels(string response, double &entry, double &sl, double &tp, int &direction)
{
   ...
   if(StringFind(upperResponse, "DECISION: LONG") >= 0 || StringFind(upperResponse, "LONG") == 0)
      direction = 1;
...
bool IsLongPatternValid(string response)
{
   ...
   bool isValid = (StringFind(upperResponse, "DECISION: LONG") >= 0 ||
                   (StringFind(upperResponse, "LONG") >= 0 && 
                    StringFind(upperResponse, "CONFIRMED") >= 0));
   
   // Vérifier les mots-clés d'invalidation
   bool isInvalid = (StringFind(upperResponse, "WAIT") >= 0 ||
                     StringFind(upperResponse, "INVALID") >= 0 ||
...
bool IsShortPatternValid(string response)
{
   ...
   bool isValid = (StringFind(upperResponse, "DECISION: SHORT") >= 0);
...
void CallGemini()
{
   ...
   if(DebugMode)
   {
      ...
      {
         if(dirTest == 1)
            Print("Distance entry/ask: ", MathAbs(ask - entryTest) / point, " points");
         else if(dirTest == -1)
            Print("Distance entry/bid: ", MathAbs(bid - entryTest) / point, " points");
      }
   ...
   if(!tradeExecuted && IsShortPatternValid(finalResponse) && AllowShort)
   {
      ...
      if(ExtractTradeLevels(finalResponse, entry, sl, tp, direction) && direction == -1)
      {
         ...
         if(!alreadyExecuted && CanTrade())
         {
            ...
            if(MathAbs(bid - entry) <= 100 * point)
            {
...
int OnInit()
{
   ...
   Print("Version 5.00 - Debug Mode: ", DebugMode ? "ON" : "OFF");
   Print("========================================");
