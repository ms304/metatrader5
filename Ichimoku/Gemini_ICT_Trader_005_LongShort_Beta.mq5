...
//| AVEC TRADING LONG ET SHORT - VERSION 5.06 CORRIGÉE               |
//+------------------------------------------------------------------+
...
// --- Paramètres d'entrée ---
input string API_KEY = "je l'aime je l'aime je l'aime";
...
//+------------------------------------------------------------------+
//| Fonction pour convertir en majuscules (renommée pour éviter conflit) |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| Calcul de la taille de lot dynamique (CORRIGÉ)                   |
//+------------------------------------------------------------------+
double CalculateLotSize(double riskDistance)
  {
   ...
   // Perte financière pour 1 lot complet
   ...
   // Arrondir au lot step du courtier
...
//+------------------------------------------------------------------+
//| Exécution d'un trade LONG (CORRIGÉ POUR SL ET TP)                |
//+------------------------------------------------------------------+
bool ExecuteLongTrade(double entry, double sl, double tp)
  {
   ...
   // NORMALISATION DES PRIX (Obligatoire pour éviter Error 130)
   sl = NormalizeDouble(sl, digits);
   tp = NormalizeDouble(tp, digits);

   // VÉRIFICATION STRICTE LONG vs Prix Actuel d'exécution
   if(sl >= ask)
     {
   ...
   // VÉRIFICATION DU STOPLEVEL COURTAGE
   if(stopLevel > 0)
     {
...
//+------------------------------------------------------------------+
//| Exécution d'un trade SHORT (CORRIGÉ POUR SL ET TP)               |
//+------------------------------------------------------------------+
bool ExecuteShortTrade(double entry, double sl, double tp)
  {
   ...
   // VÉRIFICATION STRICTE SHORT vs Prix Actuel d'exécution
   if(sl <= bid && sl > 0)
     {
...
//+------------------------------------------------------------------+
//| Gestion Breakeven et Trailing (TOTALEMENT REFAITE ET SÉCURISÉE)  |
//+------------------------------------------------------------------+
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      // Execution modif SL si pertinent et écart minimum de 1 point 
      if(slChanged && MathAbs(newSL - sl) >= point)
        {
         ...
         if(trade.ResultRetcode() == TRADE_RETCODE_DONE)
           ...
           {
            Print("Erreur mise à jour SL: ", trade.ResultRetcode());
           }
        }
     }
  }
...
//+------------------------------------------------------------------+
//| Extraction améliorée des niveaux de trade (AVEC CHECK TP LONG/S) |
//+------------------------------------------------------------------+
bool ExtractTradeLevels(string response, double &entry, double &sl, double &tp, int &direction)
  {
   ...
   // Vérifier que tous les niveaux ont été extraits
   ...
   // VÉRIFICATIONS STRICTES DE LOGIQUE EXTRAITE
   // Long : Le SL doit être sous l'entrée, le TP au-dessus
   if(direction == 1 && sl < entry && tp > entry) return true;
   // Short: Le SL doit être au-dessus de l'entrée, le TP en dessous
   if(direction == -1 && sl > entry && tp < entry) return true;

   if(DebugMode) Print("Échec validation logique: direction=", direction, " sl=", sl, " entry=", entry, " tp=", tp);
   return false;
  }
...
bool IsLongPatternValid(string response)
  {
   string upperResponse = ToUpperCase(response);
   if(StringFind(upperResponse, "DECISION: WAIT") >= 0) return false;
...
//+------------------------------------------------------------------+
//| Fonctions Utilitaires (GetData, Clean, ExtractFullResponse)      |
//+------------------------------------------------------------------+
...
void CallGemini()
  {
   ...
   // Extraire les niveaux de trade et exécuter
   double entry, sl, tp;
   int direction;
   ...
   if(ExtractTradeLevels(finalResponse, entry, sl, tp, direction))
     ...
     {
      if(DebugMode) Print("Impossible d'extraire les niveaux de trade valides");
     }
...
int OnInit()
  {
   Print("EA GEMINI SMC PATTERN démarré - Version 5.06");
   ...
   if(!TerminalInfoInteger(TERMINAL_TRADE_ALLOWED))
     {
      Print("ERREUR: Veuillez autoriser le trading automatique dans les options de MT5");
      Print("Aller dans Outils -> Options -> Expert Advisors -> cocher 'Autoriser le trading automatique'");
      return INIT_FAILED;
     }
...
void OnTick()
  {
   // Gérer les positions existantes (breakeven, trailing)
   ManagePositions();
   
   // Vérifier si on doit appeler l'API Gemini
