...
//| AVEC TRADING LONG ET SHORT - VERSION 5.08 CORRIGÉE               |
//+------------------------------------------------------------------+
...
// --- Paramètres d'entrée ---
input string API_KEY = "to be tested";
...
// --- NOUVEAUX PARAMÈTRES RISK MANAGEMENT ---
input double MinRRRatio = 1.5;          // Ratio risque/récompense minimum (1:1.5)
input int MaxSLPoints = 300;            // Stop Loss maximum en points
input int MinSLPoints = 50;             // Stop Loss minimum en points
...
//+------------------------------------------------------------------+
//| VÉRIFICATION DU RATIO RISQUE/RÉCOMPENSE (NOUVEAU)                |
//+------------------------------------------------------------------+
bool ValidateRiskReward(double entry, double sl, double tp, int direction)
  {
   ...
   Print("=== VALIDATION RISQUE/RÉCOMPENSE ===");
   ...
   if(slDistance < MinSLPoints)
     {
      Print("❌ SL TROP SERRÉ: ", slDistance, " points (minimum: ", MinSLPoints, ")");
   ...
   if(slDistance > MaxSLPoints)
     {
      Print("❌ SL TROP LARGE: ", slDistance, " points (maximum: ", MaxSLPoints, ")");
   ...
   if(rrRatio < MinRRRatio)
     {
      Print("❌ MAUVAIS RATIO RR: ", DoubleToString(rrRatio, 2), " (minimum requis: ", MinRRRatio, ")");
...
//+------------------------------------------------------------------+
//| Calcul de la taille de lot dynamique avec risque max (MODIFIÉ)   |
//+------------------------------------------------------------------+
double CalculateLotSize(double riskDistance)
  {
   ...
   // Appliquer la limite MaxRiskPerTrade
...
//+------------------------------------------------------------------+
//| Exécution d'un trade LONG (AVEC VALIDATION RR)                   |
//+------------------------------------------------------------------+
bool ExecuteLongTrade(double entry, double sl, double tp)
  {
   ...
   // ✅ NOUVEAU: Valider le ratio RR avant tout
   if(!ValidateRiskReward(entry, sl, tp, 1))
     {
      Print("❌ Trade LONG refusé - Mauvais ratio risque/récompense");
      return false;
     }
   ...
   if(trade.Buy(lot, _Symbol, ask, sl, tp, "Gemini SMC Long"))
     {
      ...
      LogTrade(StringFormat("LONG ENTRY: Prix=%.5f, SL=%.5f, TP=%.5f, Lot=%.2f, RR=%.2f, Ticket=%d", ask, sl, tp, lot, (tp-entry)/(entry-sl), ticket));
      return true;
     }
   else
     {
...
//+------------------------------------------------------------------+
//| Exécution d'un trade SHORT (AVEC VALIDATION RR)                  |
//+------------------------------------------------------------------+
bool ExecuteShortTrade(double entry, double sl, double tp)
  {
   ...
   if(!ValidateRiskReward(entry, sl, tp, -1))
     {
      Print("❌ Trade SHORT refusé - Mauvais ratio risque/récompense");
      return false;
     }
   ...
   if(trade.Sell(lot, _Symbol, bid, sl, tp, "Gemini SMC Short"))
     {
      ...
      LogTrade(StringFormat("SHORT ENTRY: Prix=%.5f, SL=%.5f, TP=%.5f, Lot=%.2f, RR=%.2f, Ticket=%d", bid, sl, tp, lot, (entry-tp)/(sl-entry), ticket));
      return true;
     }
   else
     {
...
void CallGemini()
  {
   ...
   string url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=" + API_KEY;
   ...
   string prompt =
      ...
      "4. Entry should be near current price (within 50 points/pips).\n\n" +
      "CRITICAL RISK RULES:\n" +
      "- SL must be between 50 and 300 points from entry\n" +
      "- TP must provide at least 1.5x reward vs risk (RR >= 1.5)\n" +
      "- Example GOOD SHORT: ENTRY 4865, SL 4875 (100pts risk), TP 4845 (200pts reward) -> RR = 2.0\n" +
      "- Example BAD SHORT: ENTRY 4865, SL 4890 (250pts risk), TP 4855 (100pts reward) -> RR = 0.4 (REJECT)\n\n" +
      "If a LONG setup is confirmed with entry near current price and good RR, respond EXACTLY:\n" +
      "DECISION: LONG\nENTRY: X.XX\nSL: X.XX\nTP: X.XX\nREASON: brief explanation\n\n" +
      "If a SHORT setup is confirmed with entry near current price and good RR, respond EXACTLY:\n" +
      "DECISION: SHORT\nENTRY: X.XX\nSL: X.XX\nTP: X.XX\nREASON: brief explanation\n\n" +
...
int OnInit()
  {
   Print("EA GEMINI SMC PATTERN démarré - Version 5.08");
   ...
   Print("SL min: ", MinSLPoints, " points");
   Print("SL max: ", MaxSLPoints, " points");
   Print("Risque max/trade: ", MaxRiskPerTrade, "%");
