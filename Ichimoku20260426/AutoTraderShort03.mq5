...
//+------------------------------------------------------------------+
input double   Niveau_Declenchement = 4558.94;  // Prix de déclenchement (clôture bougie <)
input double   TakeProfit           = 4550.08;  // Take Profit (en dessous du prix d'entrée)
...
int OnInit()
{
    ...
    if(StopLoss == 0)
    {
        double prix_actuel_simule = SymbolInfoDouble(_Symbol, SYMBOL_BID);
        double distance_tp_simulee = prix_actuel_simule - TakeProfit;
...
void PrendreTradeShort()
{
    ...
    if(sl_final == 0)
    {
        // Pour un SHORT : TP est plus BAS que l'entrée
        // Distance TP = entrée - TP
        double distance_tp = prix_actuel - tp;
        
        // Vérifier que le TP est bien en dessous (distance positive)
        if(distance_tp <= 0)
        {
            Print("❌ ERREUR: Take Profit (", tp, ") n'est pas en dessous du prix d'entrée (", prix_actuel, ")");
            Print("   Le TP doit être inférieur au prix d'entrée pour un SHORT!");
            return;
        }
    ...
    request.tp          = tp;                    // Take Profit (en dessous)
    request.sl          = sl_final;              // Stop Loss (au-dessus)
