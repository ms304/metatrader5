...
//--- Contrôle one-shot
bool geminiAlreadySent = false; // true après le premier envoi → plus jamais d'appel

//+------------------------------------------------------------------+
int OnInit()
{
   EventSetTimer(10);
   Print("Scanner SSB/Kijun + Gemini one-shot démarré.");
...
void OnTimer()
{
   ...
   // 1. Scan complet de tous les actifs
   for(int i = 0; i < total; i++)
      ScanSymbol(SymbolName(i, true), currentTF);

   // 2. Après la première boucle complète :
   //    si des alertes ont été collectées ET que Gemini n'a jamais été appelé → envoyer
   if(!geminiAlreadySent)
   {
      ...
      // Dans tous les cas, on verrouille : plus aucun appel Gemini après ça
      geminiAlreadySent = true;
   }
}

//+------------------------------------------------------------------+
//| Ajoute une alerte dans la file                                   |
//+------------------------------------------------------------------+
...
void FlushToGemini()
{
   string prompt = "Tu es un expert Ichimoku et trading. "
                   "Voici les alertes de proximité détectées sur différents actifs. "
   ...
   // Affichage ligne par ligne dans le journal
   string lines[];
   ...
   // Remise à zéro de la file (l'envoi lui reste verrouillé via geminiAlreadySent)
   pendingPromptLines = "";
   pendingCount       = 0;
}
...
void ScanSymbol(string symbol, ENUM_TIMEFRAMES tf)
{
   ...
   if((dist_ssb_1 > InpProximity) && (dist_ssb_0 <= InpProximity))
   {
      if(!AlreadyAlerted(symbol, time_buffer[0], tf, "SSB"))
      {
         ...
         // Empile dans la file seulement si Gemini pas encore envoyé
         if(!geminiAlreadySent)
