...
//--- Inputs Gemini
input string GeminiAPIKey = "AIzaSyBusP2MaQsTkv1uY0T7qfX56W4Q7wpE4Ko";
...
struct IchimokuData {
   ...
   double close;
   bool   valid;
   string price_vs_cloud;   // "AU-DESSUS", "DANS", "EN-DESSOUS"
   bool   bullish_cloud;
   bool   tk_cross_bull;
...
IchimokuData GetIchimokuData(string symbol, ENUM_TIMEFRAMES tf)
{
   ...
   if     (d.close > cloud_top) d.price_vs_cloud = "AU-DESSUS du nuage";
   else if(d.close < cloud_bot) d.price_vs_cloud = "EN-DESSOUS du nuage";
   else                         d.price_vs_cloud = "DANS le nuage";
      ...
      CopyBuffer(handle, SENKOUSPANB_LINE, InpKijun, 1, ssb_past) == 1)
   {
      ...
      if     (d.chikou > cloud_top_past) d.chikou_vs_cloud = "AU-DESSUS du nuage decale";
      else if(d.chikou < cloud_bot_past) d.chikou_vs_cloud = "EN-DESSOUS du nuage decale";
      else                               d.chikou_vs_cloud = "DANS le nuage decale";
   }
   else
                ...
                string level_type, string direction)
{
   ...
   Print("Prix:            ", DoubleToString(d_current.close,  5));
