//+------------------------------------------------------------------+
//|                            KijunScanner_Massive_v4.6_Fixed.mq5   |
...
//+------------------------------------------------------------------+
int OnInit() {
   // Déterminer l'UT à scanner
   g_currentTF = (InpScanTimeframe == PERIOD_CURRENT) ? _Period : InpScanTimeframe;
   
   // --- CRUCIAL : RÉINITIALISATION TOTALE ---
   ResetScannerState();
   
   // 1. Initialisation de la liste des symboles
   InitSymbolsList();
   ...
   PrintFormat("Scanner Initialisé sur %s | %d symboles.", EnumToString(g_currentTF), g_totalSymbols);
...
//+------------------------------------------------------------------+
//| Nettoyage à chaque changement (UT ou suppression)                |
//+------------------------------------------------------------------+
void ResetScannerState() {
   EventKillTimer();
   
   // Supprimer les objets graphiques
   ObjectsDeleteAll(0, g_prefix);
   
   // Libérer les handles existants pour éviter les fuites de mémoire
   ...
   // Remise à zéro des index et tableaux
   g_currentIdx = 0;
   g_accCount = 0;
   g_displayCount = 0;
   g_lastRowCount = 0;
   ArrayFree(g_accumulatedResults);
   ArrayFree(g_finalResultsForDisplay);
...
void OnTimer() {
   ...
   for(int i = g_currentIdx; i < limit; i++) {
      ...
      // Création du handle si inexistant pour la nouvelle UT
      if(g_symbolData[i].ichimokuHandle == INVALID_HANDLE) {
...
//+------------------------------------------------------------------+
//| Logique et Initialisation (Identique à la v4.6)                  |
//+------------------------------------------------------------------+
