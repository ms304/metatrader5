...
void CheckLogic(int symIdx, double lineVal, double t, double k, double a, double b, 
   ...
   // 1. Chercher si l'actif est déjà dans la liste (Mémoire)
   int foundIdx = -1;
   ...
   // 2. Si déjà détecté par le passé, on met à jour les valeurs au fil du temps (MAJ Constante)
   if(foundIdx != -1) {
      g_accumulatedResults[foundIdx].prc = price;
      g_accumulatedResults[foundIdx].lvl = lineVal; 
      g_accumulatedResults[foundIdx].diff = dev;
      // On ne relance pas ManageAlerts ici pour éviter de spammer une fois la détection faite
   }
   // 3. Sinon, on vérifie si c'est une NOUVELLE détection (Seuil + Alignement)
   else {
      if(MathAbs(dev) <= InpThresholdPercent) {
         ...
         if(isAligned) {
            ...
            g_accumulatedResults[newIdx].prc = price;
            g_accumulatedResults[newIdx].lvl = lineVal; 
            g_accumulatedResults[newIdx].diff = dev;
